...
# Описание бота.
Бот проводит розыгрыши, данных которых он получает с backend-сервера и начинает проводить розыгрыши среди пользователей телеграм, которые выполняют условия участия. Бот работает на webhook соединениях. Для реализации и запуска бота, будут необходимы домены, которые будут корректно зарегистрированы по IP VPS серверу. 
...
Для запуска бота на хосте, будет необходим провести настройки для прокси-сервера Nginx, и добавить сертификаты для доменов с помощью Let's Encrypt. 
